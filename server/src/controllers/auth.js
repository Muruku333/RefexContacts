...
const authController = {
    ...
    try {
      ...
      if (existingUser.length > 0) {
        ...
        if (compare) {
          ...
          const history = await loginHistoryModel.createLoginHistory({
            user_id,
            status: "Logged-In",
          });
          const token = jwt.sign(
            { user_id, session_id: history.insertId },
            APP_KEY
          );
  ...
  logout: async (req, res) => {
    try {
      ...
      const result = await loginHistoryModel.updateStatusById(
        data.session_id,
        "Logged-Out"
      );
  ...
  resetPassword: async (req, res) => {
    try {
      ...
      const result = await userModel.updateUserById(decoded.user.id, {
