...
const usersController = {
    getAllUsers : async (req, res)=>{
        try{
            ...
            console.log(error);
            return Response.responseStatus(res,500,'Internal server error',{ error: error.message.message});
        }
    },
    ...
    createUser : async (req, res)=>{
        try {
            ...
            const { firstName, lastName, email, mobileNumber, password = DEFAULT_PASSWORD, userType = Type.User } = req.body;
            const user = {
            ...
            if (result_email.length > 0) {
                ...
                if(createdUser.insertId > 0){
                    ...
                    return Response.responseStatus(res,201,"User Created Successfully",newUser);
                }else{
    ...
    updateUserByUserId : async (req, res) => {
        ...
        const user_id=req.params.user_id;
        const { firstName, lastName, email, mobileNumber, password, isActive, userType} = req.body;
        const user = {
            first_name: firstName, 
            last_name: lastName,
            mobile_number: mobileNumber,
